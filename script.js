...
const AppData = {
    state: {
        ...
        timetable: [],
        timerDuration: 25 // default duration in minutes
    },
...
// Pomodoro Timer Logic with Customization
const Timer = {
    interval: null,
    timeLeft: 0, 
    isRunning: false,
    ...
    init() {
        ...
        this.durationButtons = document.querySelectorAll('.toggle-duration');
        ...
        this.durationButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                if(e.target.id === 'btn-custom-time') {
        ...
        this.setDuration(AppData.state.timerDuration);
        this.highlightSavedDurationButton();
    ...
    setDuration(minutes) {
        ...
        AppData.state.timerDuration = minutes;
        AppData.save();
    ...
    highlightSavedDurationButton() {
        const savedDuration = AppData.state.timerDuration;
        let found = false;
        this.durationButtons.forEach(btn => {
            btn.classList.remove('active');
            if(parseInt(btn.dataset.time) === savedDuration) {
    ...
    showCustomTimeModal() {
        AppUI.dom.modalContent.innerHTML = `
            <h3>Custom Focus Time</h3>
            <p class="small-text" style="margin-bottom:12px;">Enter duration in minutes</p>
            <input type="number" id="custom-minutes" placeholder="e.g. 45" autocomplete="off" min="1" max="180">
            <div style="display:flex; gap:10px; margin-top:16px;">
                <button class="glass-btn primary" id="btn-save-custom" style="flex:1;">Set</button>
                <button class="glass-btn" id="btn-cancel-custom" style="flex:1;">Cancel</button>
            </div>
        `;
        AppUI.dom.modalOverlay.classList.add('active');

        document.getElementById('btn-cancel-custom').addEventListener('click', () => {
            AppUI.dom.modalOverlay.classList.remove('active');
            this.highlightSavedDurationButton(); 
        });
        ...
        document.getElementById('btn-save-custom').addEventListener('click', () => {
            ...
            if(mins > 0 && mins <= 300) {
                ...
                this.updateActiveButton(document.getElementById('btn-custom-time'));
                AppUI.dom.modalOverlay.classList.remove('active');
            } else {
                alert("Please enter a valid time between 1 and 300 minutes.");
            }
        });
    },
    ...
    start() {
        ...
        this.interval = setInterval(() => {
            if(this.timeLeft > 0) {
                ...
                if(AppData.state.studyTimeToday % 60 === 0) AppData.save(); 
            } else {
                this.complete();
            }
        }, 1000);
    ...
    pause() {
        ...
        if(this.timeLeft > 0) this.status.innerText = "Paused";
        AppData.save();
